---
title: "Visualización de datos para la transferencia \\de resultados en distintos ámbitos científicos"
subtitle: "INIE - Formación PDI, UniOvi"  
author: "Noelia Rico - Departamento de Informática"
date: '5-6 JULIO 2021'
output:
...
- Aproximación teórica sobre como construir gráficos.
...
![Relación entre bill_length_mm y bill_depth_mm](./imgs/pico.png)
...
¿En qué isla hay más de la especie Adelie?
  - El gráfico de la derecha nos permite una comparación mejor
...
Almacenamos en `penguins_total` una variación del conjunto de datos original.
...
El objetivo es obtener el siguiente gráfico:
...
Vamos a darle transparencia utilizando la estética `alpha`
...
Rellenar en función del sexo del pingüino
...
Añadir una nueva capa con una etiqueta para indicar macho y hembra
...
Cambiar el tema. Pero ojo que así no funciona
...
ggplot(penguins, aes(x = bill_length_mm)) +
  ...
  theme_minimal() +
  theme(legend.position = "none",
        text = element_text(family = "Times", size = 16)) # aquí en windows TT Times New Roman
```
...
Nota, las fuentes dependen del sistema operativo que estés utilizando:
...
Indicar saltos de línea en las etiquetas
...
Transformación del eje y: cambiar etiquetas
...
Transformación del eje x: añadir más breaks
...
Introducir líneas verticales para mostrar la media
...
Añadir medias filtrando por sexo
...
.footnote[
Otras variaciones más complejas del gráfico de barras que se están popularizando: [Circular barl plots]()https://www.r-graph-gallery.com/297-circular-barplot-with-groups.html)]
...
Dividiendo en función a dos variables
...
- Como vemos, también podríamos hacer comparaciones de dos variables con `facet_wrap`
...
.pull-left[
- Cuadrícula de celdas cuyos *colores* representan los datos.
  - También pueden utilizarse formas, iconos... y combinaciones de estos
- Buena forma de mostrar el progreso hacia un objetivo. 
- También útil para representar la división de un total por grupos.
...
Ejemplo: *¿Cuántos diamantes hay atendiendo a su tipo de corte?*
...
Contar el número de cuadros correspondientes
...
waffle(
  diamonds %>%
    ...
    rows = 5, xlab = "Diamantes según su corte")
```
...
diamonds %>%
  ...
  title = "Diamantes según su corte",
...
- Cada ejecución elige aleatoriamente
...
La segunda versión del paquete genera una imagen en formato `.html` con el que se puede interactuar.
...
Al pasar el ratón por encima muestra el número de apariciones de la palabra.
...
El objetivo es obtener este gráfico:
...
- Crear tres gráficos con al menos cinco capas cada uno.
- Justificar la creación de estos gráficos: 
  - ¿Qué quieren mostrar? 
  - ¿Por qué se ha elegido ese tipo de gráfico? 
  - ¿Por qué se han elegido esas aesthetics para mapear las variables y no otras?

- Entregar:
  - Archivo `.R` con el código generado y archivo de datos en caso de que el script no los descargue de forma remota.
  - Explicación de la motivación tras los gráficos generados (en un archivo pdf independiente o como comentarios de código del archivo `.R`).

**DEADLINE**: Miércoles 21 de Julio 2021, 23:59.